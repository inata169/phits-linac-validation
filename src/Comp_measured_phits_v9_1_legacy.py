...
# 共通グリッドへの再サンプル（cm単位）
def _resample_to_common_grid(x_ref_cm, y_ref, x_eval_cm, y_eval, grid_step_cm=0.1):
    try:
        if grid_step_cm is None or grid_step_cm <= 0:
            return None
        x_min = max(np.nanmin(x_ref_cm), np.nanmin(x_eval_cm))
        x_max = min(np.nanmax(x_ref_cm), np.nanmax(x_eval_cm))
        if not np.isfinite([x_min, x_max]).all() or (x_max - x_min) <= (2 * grid_step_cm):
            return None
        n_steps = int(np.floor((x_max - x_min) / grid_step_cm)) + 1
        x_grid = x_min + np.arange(n_steps + 1) * grid_step_cm
        y_ref_g = np.interp(x_grid, x_ref_cm, y_ref)
        y_eval_g = np.interp(x_grid, x_eval_cm, y_eval)
        return x_grid, y_ref_g, y_eval_g
    except Exception:
        return None
...
def main():
    ...
    # 共通グリッド間隔(cm)の取得（config.ini [Processing].resample_grid_cm）
    try:
        grid_step_cm = float(config.get('Processing', 'resample_grid_cm', fallback='0.1'))
    except Exception:
        grid_step_cm = 0.1

    # RMSE計算: 必要に応じて共通グリッドに再サンプルして安定化
    try:
        x_meas = np.asarray(df_measured['pos'], dtype=float)
        y_meas = np.asarray(df_measured['dose_normalized'], dtype=float)
        x_calc = np.asarray(df_phits['pos_center'], dtype=float)
        y_calc = np.asarray(df_phits['dose_smoothed'], dtype=float) if 'dose_smoothed' in df_phits else np.asarray(df_phits['dose_normalized'], dtype=float)
        rs = _resample_to_common_grid(x_meas, y_meas, x_calc, y_calc, grid_step_cm)
        if rs is not None:
            _, y1g, y2g = rs
            rmse_value = float(np.sqrt(np.mean((y1g - y2g) ** 2)))
            print(f"✳ RMSEは共通グリッド間隔 {grid_step_cm:.3f} cm 上で計算しました。")
        else:
