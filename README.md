...
スクリプトの安定性と互換性を向上させるため、以下の修正を行いました。

*   **PHITS出力ファイルの解析処理を強化:**
    *   PHITSのTallyセクション開始を示すマーカー (`[ T-Deposit ]`) の検出処理を改善。スペースの有無や大文字・小文字の違い（例: `[T-Deposit]`, `[ t - deposit ]`）があっても、セクションを正しく認識できるようになりました。
    *   Tallyセクション内のデータヘッダーとデータ本体の間に予期せぬ空行やコメント行が存在しても、データを柔軟に読み取れるようになりました。

*   **座標軸の互換性を向上:**
    *   PHITSの座標系と実測データの座標系の対応チェックを拡張し、`x-x`, `y-y`, `z-z` のように同じ軸同士の組み合わせも正常な比較対象として処理されるようになりました。

*   **実測データCSVの読み込みを強化:**
    *   CSVファイルのヘッダーに含まれる軸名（例: `X (cm)`）が、小文字 (`x (cm)`) で記述されていても、大文字・小文字を区別せずに軸を認識できるようになりました。
